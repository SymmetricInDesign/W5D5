...
def harrison_ford
  ...
  Actor.joins(:movies).select('movies.id', 'title')
...
def directed_by_one_of(them)
  ...
  # their_ids = Actor.where(name: them).pluck(:id)
  # Movie.select(:id, :title).where(director_id: their_ids)

  Actor.select('movies.id', :title).joins(:directed_movies).where(name: them)
